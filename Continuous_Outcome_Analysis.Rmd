...
# This makes sense. filtering data that has Inf, and those that don't, yeild two equal groups
...
ggplot(refined_set, aes(Setting_1, CasesAll)) + geom_point()
ggplot(final_draft, aes(Setting, CasesAll)) + geom_point()
...
sum(refined_set$CasesAll)
sum(final_draft$CasesAll)

if(sum(refined_set$RateAll) == sum(refined_set$RateAll))
  print("Yay!")
...
#plot the OBYear separately from the others, since it is a factor level variable
p9
```
...
# I see the 0, and it is there in the overview
glimpse(final_draft$OBYear)
...
glimpse(removed_0OBYear$OBYear)
ggplot(removed_0OBYear, aes(OBYear, RateAll)) + geom_point() + scale_fill_discrete(drop=FALSE) + scale_x_discrete(drop=FALSE)
...
glimpse(removeCategory$Hemisphere)
...
data_test = final_draft[-trainset,] #do the same for the test set
```
...
RMSE(data_train$fracinf, mean(data_train$fracinf))

# The code above gives me the RMSE (28.20524) by calculating the outcome versus a null model (mean(data_train$fracinf))
```
...
fit1 <- train(fracinf ~ ., data= data_train, method= "lm", trControl= fitControl, preProcess= c("center","scale"))

fit2 <- train(fracinf ~ ., data= data_train, method= "earth", trControl= fitControl, preProcess= c("center","scale"))

fit3 <- train(fracinf ~ ., data= data_train, method= "knn", trControl= fitControl, preProcess= c("center","scale"))
...
train_residuals <- residuals(fit3)

#plot and look at RSME
ggplot(add_predictions, aes(predictions, data_train$fracinf)) + geom_point() + geom_smooth(method="lm")
RMSE(predictions, data_train$fracinf)

ggplot(add_predictions, aes(predictions, train_residuals)) + geom_point() + geom_smooth(method="lm")
...
#RMSE here= 26.68. Still better than null at least.
RMSE(data_test$fracinf, predictions_finalModel)
