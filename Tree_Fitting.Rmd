...
levels(removed_emptyCat$season)
dim(removed_emptyCat)
...
# I think I am overthinking this, and I don't think this is correct.
is.na(picked_variables)
...
removed_0OBYear <- subset(picked_variables, OBYear != "0" & OBYear != "2002-2007")
removed_0OBYear <- droplevels(removed_0OBYear)

#removed_0OBYear$OBYear
#str(removed_0OBYear)

year_as_numeric <- as.numeric(as.character(removed_0OBYear$OBYear))

removed_0OBYear$OBYear <- year_as_numeric

str(removed_0OBYear)
vis_dat(removed_0OBYear)
```

Look at the data to see what else we need to do.

```{r}
summary(removed_0OBYear)
```
...
#str(further_cleaning)
#vis_dat(further_cleaning)

summary(further_cleaning$Action1)
#Get rid of the No observation
...
summary(removed_Action1No$Action1)
#removed the empty No level since that observation was dropped
...
summary(fixCountryFactors$Country)

summary(fixCountryFactors$Trans1)

#move waterborne into environmental
mergeWaterborne_environmental <- fixCountryFactors %>%
  mutate(Trans1= fct_recode(Trans1, "Environmental" = "Waterborne"))

summary(mergeWaterborne_environmental$Trans1)

summary(mergeWaterborne_environmental$season)

dropStateVar <- dplyr::select(mergeWaterborne_environmental, -c(State))
head(dropStateVar)

Vomiting <- as.factor(dropStateVar$Vomit)

makeVomit_factor <- dropStateVar %>%
  mutate(Vomiting) %>%
  select(-c(Vomit))

head(makeVomit_factor)

season_first <- makeVomit_factor %>%
  select(season, everything())

summary(season_first)
str(season_first)
vis_dat(season_first)
#season_first
```
...
p6 <- ggplot(season_first, aes(season, CasesAll)) + geom_violin()

p7 <- ggplot(season_first, aes(season, Deaths)) + geom_violin()

p8 <- ggplot(season_first, aes(season, EndMonth)) + geom_violin()

p9 <- ggplot(season_first, aes(season, Hospitalizations)) + geom_violin()

grid.arrange(p6, p7, p8, p9, nrow=2, ncol=2)


p10 <- ggplot(season_first, aes(season, MeanI1)) + geom_violin()

p11 <- ggplot(season_first, aes(season, MedianI1)) + geom_violin()

p12 <- ggplot(season_first, aes(season, StartMonth)) + geom_violin()
...
# I still need to fix these up a bit
...
p11 <- ggplot(season_first, aes(season, gg2c4)) + geom_count()
p12 <- ggplot(season_first, aes(season, Vomiting)) + geom_count()


grid.arrange(p5, p6, p7, p8, p9, p10, p11, p12, ncol=2, nrow=4)
...
set.seed(123)
trainset <- caret::createDataPartition(y = season_first$season, p = 0.7, list = FALSE)
data_train = season_first[trainset,]
data_test = season_first[-trainset,]
...
prp(fit1$finalModel, extra = 1, type = 1)
ww=17.8/2.54; wh=ww; #for saving plot
dev.print(device=png,width=ww,height=wh,units="in",res=600,file="rparttree_datatrain_mod.png") #save tree to file
```
...
fit2 = caret::train(season ~ ., data=data_train_mod, method="ranger",  trControl = fitControl, tuneGrid = tuning_grid, na.action = na.pass) 
```
...
fit3 = caret::train(season ~ ., data=data_train_mod, method="gbm", trControl = fitControl, verbose=FALSE, tuneGrid = gbmGrid) 
```
...
set.seed(1111) #makes each code block reproducible
tuning_grid <- expand.grid( .mtry = seq(1,7,by=1), .splitrule = "gini", .min.node.size = seq(2,8,by=1) )
fit4 = caret::train(season ~ ., data=data_train_mod, method="ranger",  trControl = fitControl, tuneGrid = tuning_grid, na.action = na.pass, preProcess= c("center","scale")) 
plot(fit4)
```
...
set.seed(1111) #makes each code block reproducible
tuning_grid <- expand.grid( .mtry = seq(1,7,by=1), .splitrule = "gini", .min.node.size = seq(2,8,by=1) )
fit5 = caret::train(season ~ ., data=data_train_mod, method="pda",  trControl = fitControl, na.action = na.pass, tuneLength=20) 
plot(fit5)
...
#I went ahead and created a manual calculation of accuracy before calling confusion matrix to see if I got the same thing.  And yay, I do!
...
total_accuracy <- sum(accuracy_table$accuracy/nrow(accuracy_table))

total_accuracy

confusionMatrix(accuracy_table$predictions, accuracy_table$season)
...
#Manual calculation of accuracy tht matches confusionMatrix call
...
#accuracy_table_final

total_accuracy_final <- sum(accuracy_table_final$accuracy/nrow(accuracy_table_final))

total_accuracy_final

confusionMatrix(accuracy_table_final$predictions, accuracy_table_final$season)
